...
from utils import transform_pertub



def get_fgsm_grad_align_loss(args,criterion,model,image,target,optimizer,eps):
    ...
    grad = torch.autograd.grad(loss_clean, delta, create_graph=True if double_bp else False)[0]

    grad = grad.detach()
    argmax_delta = eps * sign(grad)
    delta.data = clamp(delta.data + args.fgsm_alpha * argmax_delta, -eps, eps)
    tmp_delta = transform_pertub(args.dataset,image, delta)
    ...
    if args.grad_align_cos_lambda != 0.0:
        grad2 = get_input_grad(args,model, image, target, optimizer, eps, False, delta_init='random_uniform', backprop=True)
...
def get_input_grad(args,model, X, y, opt, eps, half_prec, delta_init='none', backprop=False):
    if delta_init == 'none':
    ...
    tmp_delta = transform_pertub(args.dataset,X, delta)
    output = model(X + tmp_delta)

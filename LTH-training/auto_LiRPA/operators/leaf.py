...
class BoundParams(BoundInput):
    ...
    def register_parameter(self, name, param):
        #if name == 'param': mofified for pruning
        if 'param' in name:
    ...
    def forward(self):
        if self.initializing:
            ...
            if hasattr(self,'param_mask'):
                self.param=self.param_orig*self.param_mask
                param=self.param
            elif hasattr(self,'popup_scores'):
                adj = GetSubnet.apply(self.popup_scores.abs(), self.k)
                # Use only the subnetwork in the forward pass.
                param = self.param * adj
            else:
                param=self.param
            return param
    ...
    def set_hydra_mode(self,mode=True):
        '''
        TODO: add crown backward mode support
        '''
        if mode==True:
            self.hydra=True
            self.popup_scores = nn.Parameter(torch.Tensor(self.param.shape))
            nn.init.kaiming_uniform_(self.popup_scores, a=math.sqrt(5))
        else:
            self.hydra=False
            del self.popup_scores
            del self.k
    ...
    def subnet_mask_out(self):
        adj = GetSubnet.apply(self.popup_scores.abs(), self.k)
        self.param.data = adj * self.param

...
class MultiAverageMeter(object):
    ...
    def reset(self):
        ...
        self.sum_sqr_meter= defaultdict(float)
    def update(self, key, val, n=1, sqr=None):
        self.lasts[key] = val
        self.sum_meter[key] += val * n
        self.counts_meter[key] += n
        if sqr!=None:
            self.sum_sqr_meter[key] += sqr*n
    def last(self, key):
    ...
    def std(self,key):
        if self.counts_meter[key] == 0:
            ...
            var = self.sum_sqr_meter[key] / self.counts_meter[key] - self.avg(key)*self.avg(key)
            logger.info('min var: {}'.format(var.min()))
            var.clip_(min=0)

...
import arguments
import math
from torchvision.datasets import SVHN,CIFAR100,FashionMNIST
...
def load_model(weights_loaded=True,compress=False,num_classes=10,greyscale=False):
    """
    Load the model architectures and weights
    """
    # You can customize this function to load your own model based on model name.
    model_ori = eval(arguments.Config['model']['name'])(num_classes=num_classes, greyscale=greyscale)
    print(model_ori)
    ...
    model_state_dict = torch.load(arguments.Config["model"]["path"], map_location=torch.device('cpu'))
    ...
    if not compress:
        ...
        print('channel_num:',channel_nums)
        model_new = eval(arguments.Config['model']['name'])(channels=channel_nums,num_classes=num_classes, greyscale=greyscale)
        for i in range(len(conv_idxs)):
            ...
            print(conv_.weight.data.shape,remain_channel_idxs,prev_remain_idx)
...
def load_eran_dataset(eps_temp=None):
    ...
    if arguments.Config["data"]["dataset"] == "CIFAR_ERAN":
        ...
        std = np.array([0.2470, 0.2435, 0.2616]).reshape(1, -1, 1, 1).astype(np.float32)
#        mean = np.array([0.4914, 0.4822, 0.4465]).reshape(1, -1, 1, 1).astype(np.float32)
...
def load_svhn_dataset(eps_temp):
    ...
    test_set = SVHN('datasets/svhn', split='test', transform=test_transform, download=True)

    # val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    test_loader = DataLoader(test_set, batch_size=200, shuffle=False, num_workers=1, pin_memory=True)
    X,labels=next(iter(test_loader))
    runnerup=None
    data_max=torch.ones(1,3,1,1)
    data_min=torch.zeros(1,3,1,1)
    eps_new=torch.ones(1,3,1,1)*eps_temp
    return X,labels,runnerup,data_max,data_min,eps_new


def load_cifar100_dataset(eps_temp):
    ...
    X,labels=next(iter(test_loader))
    runnerup=None
    mean=torch.FloatTensor([0.5071, 0.4866, 0.4409]).reshape([1,3,1,1])
    std=torch.FloatTensor([0.2673, 0.2564, 0.2762]).reshape([1,3,1,1])
    data_max=(torch.ones(1,3,1,1)-mean)/std
    data_min=(torch.zeros(1,3,1,1)-mean)/std
    eps_new=torch.ones(1,3,1,1)*eps_temp/std
    return X,labels,runnerup,data_max,data_min,eps_new

def load_FashionMNIST_dataset(eps_temp):
...
#    std=0.315
    ...
    #    train_set = Subset(CIFAR10(data_dir, train=True, transform=train_transform, download=True), list(range(45000)))
    #    test_set = Subset(CIFAR10(data_dir, train=True, transform=test_transform, download=True), list(range(45000, 50000)))
    test_set = FashionMNIST('datasets/FashionMNIST', train=False, transform=test_transform, download=True)

    # val_loader = DataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
    test_loader = DataLoader(test_set, batch_size=200, shuffle=False, num_workers=1, pin_memory=True)
    X,labels=next(iter(test_loader))
    runnerup=None
    data_max=torch.ones(1,1,1,1)*(1-mean)/std
    data_min=torch.ones(1,1,1,1)*(0-mean)/std
    eps_new=torch.ones(1,1,1,1)*eps_temp/std
    return X,labels,runnerup,data_max,data_min,eps_new
...
def load_verification_dataset(eps_before_normalization):
    ...
    # Some special model loaders.
    elif 'FashionMNIST' in arguments.Config["data"]["dataset"]:
        X, labels, runnerup, data_max, data_min, eps_new = load_FashionMNIST_dataset(eps_temp=eps_before_normalization)
    elif 'CIFAR100' in arguments.Config["data"]["dataset"]:
        X, labels, runnerup, data_max, data_min, eps_new = load_cifar100_dataset(eps_temp=eps_before_normalization)
        ...
        X, labels, runnerup, data_max, data_min, eps_new = load_eran_dataset(eps_temp=eps_before_normalization)
    elif 'SVHN' in arguments.Config["data"]["dataset"]:
        X, labels, runnerup, data_max, data_min, eps_new = load_svhn_dataset(eps_temp=eps_before_normalization)

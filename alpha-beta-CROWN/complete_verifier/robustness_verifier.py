...
def config_args():
    ...
    h = ["general"]
    arguments.Config.add_argument("--mode", type=str, default="crown-acc", choices=["verified-acc", "crown-acc", "runnerup", "clean-acc", "specify-target"],
    ...
    h = ["data"]
    arguments.Config.add_argument("--dataset", type=str, default="CIFAR", choices=["MNIST", "CIFAR", "CIFAR100","FashionMNIST", "SVHN", "CIFAR_SDP_FULL", "CIFAR_RESNET", "CIFAR_SAMPLE", "MNIST_SAMPLE", "CIFAR_ERAN", "MNIST_ERAN",
...
def get_statistics(model, image, true_label, eps, data_min, data_max, batch_size, method="CROWN"):
    ...
    clean_acc = n_correct/true_label.nelement()
    print(f'{n_correct} examples are correct, clean acc: {clean_acc}, image range ({image.min()}, {image.max()})')
    ...
    print(f"{method} clean acc:{clean_acc * 100:.2f}%, verified acc: {verified/N * 100:.2f}%, {verified} verified, time used: {time_used:.2f} min, max_mem: {torch.cuda.max_memory_allocated()/(1024*1024*1024):.2f}GB",)
...
def main():
    ...
    if arguments.Config["data"]["dataset"]=='CIFAR100':
        num_class=100
    else:
        num_class=10
    model_ori = load_model(weights_loaded=True,compress=arguments.Config["model"]["compress"],num_classes=num_class,greyscale=("MNIST" in arguments.Config["data"]["dataset"]))
...
#    if 0:
    ...
    print("=====================>")
    if arguments.Config['general']['mode']=='crown-acc':
...
#    print(f'max_mem:{max_mem/1024.0:.2f}GB')
